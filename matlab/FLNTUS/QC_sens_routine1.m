...
for i = 1:length(files)              
    ...
    fl_cnts = ncread(filename, 'ECO_FLNTUS_CHL');
    ...
    if med > 1
        ...
        continue;
           ...
           for l=1:length(ind)
               ...
               s_time(l,1) = nanmean(time(cnt1:ind(l)));
               
               % contract T and S
               s_sal(l,1) = nanmean(sal(cnt1:ind(l)));
               s_temp(l,1) = nanmean(temp(cnt1:ind(l)));

               % contract the QC vectors
               % preserve the highest flag observed for each burst
               s_fl_qc(l,1) = nanmax(fl_qc(cnt1:ind(l)));
               s_bb_qc(l,1) = nanmax(bb_qc(cnt1:ind(l)));
